...
namespace ManagedPlugin
{
    ...
    public abstract class PluginBase : IIdaPlugin
    {
        ...
        protected abstract void DoRun(string name);

        /// <summary>Make sure out private log file path is initialized.</summary>
        private static void EnsurePrivateLogFile()
        {
            if (null == _privateLogFile)
            {
                _privateLogFile = Environment.ExpandEnvironmentVariables(@"%LOCALAPPDATA%\idaplug\logs\ManagedPlugin.log");
            }
            return;
        }
        ...
        /// <returns></returns>
        private static StreamWriter GetPrivateLogWriter()
        {
        ...
        private IntPtr GetSynchronizationEvent()
        {
            try
            {
                return Native.CreateEvent(IntPtr.Zero, true, false, SynchronizationEventName);
            }
            catch (Exception e)
            {
        ...
        private void InitializeId()
        {
            ...
            GuidAttribute myGuidAttribute = Assembly.GetExecutingAssembly().GetCustomAttribute(typeof(GuidAttribute)) as GuidAttribute;
            if (null == myGuidAttribute)
            {
                throw new InvalidOperationException("No Guid attribute found on ManagedPlugin assembly.");
            }
            ...
            for (int frameIndex = 0; frameIndex < framesCount; frameIndex++)
            {
                ...
                if (null == guidAttribute)
                {
                    throw new InvalidOperationException(string.Format("No Guid attribute could be found on calling assembly '{0}'.", candidateAssembly.FullName));
                }
        ...
        public void Run(string name)
        {
            ...
            finally
            {
                lock (this)
                {
                    ...
                    Native.SetEvent(_synchronizer);
                    Native.CloseHandle(_synchronizer);
        ...
        public static void WriteDebugMessage(string pattern, params object[] parameters)
        {
            using (StreamWriter writer = GetPrivateLogWriter())
            {
                writer.WriteLine(pattern, parameters);
                Interactivity.Message(string.Format(pattern, parameters));
            }
        ...
        private static string _privateLogFile = null;
