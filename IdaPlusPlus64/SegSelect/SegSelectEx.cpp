...
SegSelectEx_segments *SegmentDialog::getSelected()
{
    SegSelectEx_segments *result = new SegSelectEx_segments();
    int count = segmentTable->rowCount();
    for (int i = 0; i < count; i++)
    {
        if (QTableWidgetItem *item = segmentTable->item(i, 0))
            if (item->checkState() == Qt::Checked)
                result->push_back(getnseg(i));
    }

    return(result);
}

SegSelectEx_segments* segmentList = NULL;

segment_t* _cdecl SegSelectEx_getNthSelectedSegment(int n)
{
    segment_t * segment = segmentList != NULL ? (n < segmentList->size() ? segmentList->at(n) : NULL) : NULL;
    return segment;
}

int _cdecl SegSelectEx_getSelectedSegmentsCount()
{
    return segmentList != NULL ? segmentList->size() : 0;
}

// Do segment selection dialog
void _cdecl SegSelectEx_select(UINT flags, LPCSTR title, LPCSTR styleSheet, LPCSTR icon)
{
    try
    {
...
void _cdecl SegSelectEx_processIdaEvents()
{ 
