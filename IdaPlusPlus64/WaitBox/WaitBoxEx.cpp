...
void _cdecl WaitBoxEx_show(LPCSTR titleText, LPCSTR labelText, LPCSTR styleSheet, LPCSTR icon)
{
    if (!showState)
    {
        try
...
void _cdecl WaitBoxEx_showDefault()
{
    if (!showState)
    {
        try
        {
            // Create the dialog
            if (prgDlg = new MyQProgressDialog("Class Informer", "Please wait..", STYLE_PATH "progress-style.qss", STYLE_PATH "icon.png"))
            {
                // Taskbar progress
                showState = isUpdateReady = TRUE;
                TaskProgress::start(getIdaHwnd());
...
BOOL _cdecl WaitBoxEx_isShowing()
{
    return(showState && (prgDlg && !prgDlg->isCanceled()));
}

// Returns TRUE if wait box is up
BOOL _cdecl WaitBoxEx_isUpdateTime()
{ 
    return(isUpdateReady); 
}

// Set the label text
void _cdecl WaitBoxEx_setLabelText(LPCSTR labelText)
{
    try
    {
        if (prgDlg && labelText)
...
void _cdecl WaitBoxEx_processIdaEvents() 
{ 
...
BOOL _cdecl WaitBoxEx_updateAndCancelCheck(int progress)
{
    if (showState)
    {
        if (prgDlg)
